...
namespace SMPD
{
    ...
    {
        private readonly List<MapleSample> _mapleSamples = new MapleOakExampleFileReader().ReadSamples().ToList();
        private readonly List<double[]> _acer;
        private readonly List<double[]> _quercus;
        ...
        private void bootsrapttest_Click(object sender, EventArgs e)
        {
            ...
            {
                case "kNN":
                    this.bootstrapresult.Text = new Bootstrap<KnnClassifier>(this._mapleSamples, int.Parse(this.textBox9.Text)).Test(int.Parse(this.bootstrapparts.Text), int.Parse(this.bootsrapcount.Text)).ToString(CultureInfo.InvariantCulture) + '%';
                    break;
                case "NN":
                    this.bootstrapresult.Text = new Bootstrap<NNClassifier>(this._mapleSamples, 1).Test(int.Parse(this.bootstrapparts.Text), int.Parse(this.bootsrapcount.Text)).ToString(CultureInfo.InvariantCulture) + '%';
                    break;
                case "NM":
                    this.bootstrapresult.Text = new Bootstrap<NearestMeanClassifier>(this._mapleSamples, 1).Test(int.Parse(this.bootstrapparts.Text), int.Parse(this.bootsrapcount.Text)).ToString(CultureInfo.InvariantCulture) + '%';
                    break;
            }
        }
    }
